<?php 
    class Employee {
        ...
        public static function add(){
            ...
            $query = Db::fetch(self::$tbl_employees, "", "employee_number = ? AND card_id = ? AND fingerprint_number = ?", array($employee_number, $card_id, $fingerprint_number), "", "", "");
            ...
            if(Db::count($query)){
                ...
                Db::insert(self::$tbl_employees, array("employee_number", "card_id", "fingerprint_number", "worker_type", "job_position", "shifting_type", "first_name", "last_name", "middle_name", "birth_date", "age", "gender", "civil_status", "full_address", "email", "contact_number", "contact_person", "contact_person_number", "relationship", "duration_date", "start_date", 
        ...
        public static function fetchList(){
            $query = Db::fetch(self::$tbl_employees, "", "", "", "", "", "");

            $limit = $_GET['start'].', '.$_GET['length'];
            if($_GET['length'] != -1){
                $query = Db::fetch(self::$tbl_employees, "", "", "", "", $limit, "");
            }

            if(!empty($_GET['search']['value'])){
                $like_val = $_GET['search']['value'];

                $query = Db::fetchLike(self::$tbl_employees, "employee_number", $like_val);
            }

            $listData = array();
            while($tbl_employee = Db::assoc($query)){
                $dataRow = array();
                $dataRow[] = '<input type="checkbox" name="selector[]" id="selector[]" value="' . $tbl_employee['id'] . '" />';
                $dataRow[] = $tbl_employee['employee_number'];
                $dataRow[] = $tbl_employee['first_name'].' '. $tbl_employee['last_name'];
                $dataRow[] = $tbl_employee['card_id'];
                $dataRow[] = $tbl_employee['fingerprint_number'];
                $dataRow[] = $tbl_employee['contact_number'];
                $dataRow[] = $tbl_employee['shifting_type'];
                $dataRow[] = $tbl_employee['job_position'];
                $dataRow[] = '<button type="button" name="update" id="'.$tbl_employee['id'].'" class="btn btn-success update"><i class="material-icons">edit</i></button> <button type="button" name="info" id="'.$tbl_employee['id'].'" class="btn btn-info info"><i class="material-icons">info</i></button> 
            <button type="button" name="delete" id="'.$tbl_employee['id'].'" class="btn btn-danger delete"><i class="material-icons">delete</i></button>';
